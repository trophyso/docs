...
"og:description": Use metric, API, streak and composite achievements to reward users for continued progress and to encourage them to discover new parts of your app.
icon: trophy
---
...
<Tip>
  Use Trophy's [metric analytics](/en/platform/metrics#metric-analytics) to compare
...
Trophy offers four types of achievements, [Metric](#metric-achievements), [API](#api-achievements), [Streak](#streak-achievements) and [Composite achievements](#composite-achievements), detailed below.

### Metric Achievements {#metric-achievements}

Metric achievements are tied to [Metrics](/en/platform/metrics) and are best used when you want to incentivize users to take the same action over and over again.
...
Since these achievements are directly tied to the _Flashcards Flipped_ metric, Trophy will automatically track when users unlock these achievements as they [increment the metric](/en/platform/events#tracking-metric-events).

When achievements are unlocked, Trophy includes information about the unlocked achievements in the [Event API](/en/api-reference/endpoints/metrics/send-a-metric-change-event) response, and automatically triggers [Achievement Emails](/en/platform/emails#achievement-emails) if configured.
...
Just like metric achievements, API achievements can also trigger automated [Achievement Emails](/en/platform/emails#achievement-emails) if configured.

### Streak Achievements {#streak-achievements}

Streak achievements are directly tied to a user's [Streak](/en/platform/streaks) and are automatically unlocked when users reach a particular streak length.
...
Composite achievements are useful for creating progression pathways and encouraging users to engage across different areas of your app. Just like other achievement types, they can trigger [Achievement Emails](/en/platform/emails#achievement-emails) when unlocked.
...
- Choosing **API Call** will mean the achievement will only be unlocked when explicitly marked as completed by your code through a request to the [complete achievement API](/en/api-reference/endpoints/achievements/mark-an-achievement-as-completed).
...
- If you chose the **API Call** trigger, you'll need to choose a unique reference `key` you'll use to complete the achievement via the [API](/en/api-reference/endpoints/achievements/mark-an-achievement-as-completed).
...
- To limit a **Metric** achievement to only apply to events with specific [custom event attributes](/en/platform/events#custom-event-attributes), select an attribute and enter a value in the **Event Attribute** section.

- To limit any type of achievement to only apply to a user with one or more specific [custom user attributes](/en/platform/users#custom-user-attributes), add attributes and the desired values in the **User Attributes** section.
...
If you're using metric achievements, there's no need to explicitly _complete_ achievements. Once you've set up [metric tracking](/en/platform/events#tracking-metric-events) in your code, all achievements linked to the metric will be automatically tracked.
...
However if you're using any API achievements, you will have to mark them as completed for each user as appropriate. To do this, you can use the [Complete Achievement API](/en/api-reference/endpoints/achievements/mark-an-achievement-as-completed) using the `key` of the achievement you want to complete.
...
<Tip>
  Check out our [full guide](/en/guides/how-to-build-an-achievements-feature) on
  adding an achievements feature to your app for more details.
</Tip>

### All Achievements {#all-achievements}

To display a high-level overview of all achievements users can complete, use the [all achievements endpoint](/en/api-reference/endpoints/achievements/all-achievements). Use this data to build UI that gives users an idea of the progression pathways within your application.
...
If instead you're building user-specific UI elements, then use the [user achievements endpoint](/en/api-reference/endpoints/users/get-a-users-completed-achievements) to return achievements a specific user has completed.
...
If you have achievements set up for any of your [Metrics](/en/platform/metrics), then the metric analytics page displays a chart that shows you the current progress of all Users as follows:
...
<AccordionGroup>
  <Accordion title="What type of achievements should I use?">
    ...
    Use Trophy's [metric analytics](/en/platform/metrics#metric-analytics) to compare the retention of each user interaction, then configure achievements around these interactions to maximize retention impact.
