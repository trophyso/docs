...
<Tip>
  ...
  triggers. See the dedicated [points docs](/en/platform/points#points-triggers)
...
<Note>
  If you use [Points Levels](/en/platform/points#points-levels), the `xp` object can include a **`level`** field **only when** the user's tier changed on this request; it may be omitted when their level stayed the same. See the [metric change API reference](/en/api-reference/endpoints/metrics/send-a-metric-change-event) for the full response schema.
</Note>
...
We can use the response of the [metric change API](/en/api-reference/endpoints/metrics/send-a-metric-change-event) to show users pop-up notifications when users are awarded new XP.
...
### Displaying User XP {#displaying-user-xp}

To fetch a users XP use the [user points API](/en/api-reference/endpoints/users/get-a-users-points).

<UserPointsRequest />

This API returns data on the user's total XP but can be configured to also return between 1 and 100 of the user's most recent XP awards by using the `awards` [query parameter](/en/api-reference/endpoints/users/get-a-users-points#parameter-awards).
...
### User XP Chart {#user-xp-chart}

The [user points summary API](/en/api-reference/endpoints/users/get-a-users-points-summary) returns chart-ready historical data for displaying how a user's XP has changed over time.
