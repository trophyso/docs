...
Once you've created your metric, head to the [leaderboards page](https://app.trophy.so/leaderboards) and create the leaderboards you want. You can find all the details on the types of leaderboards and the different use cases in the [leaderboards docs](/en/platform/leaderboards).
...
<Tip>
  ...
  our [full guide](/en/guides/how-to-build-an-xp-feature).
</Tip>
...
We can use the response of the [metric change API](/en/api-reference/endpoints/metrics/send-a-metric-change-event) to show pop-up notifications (or 'toasts') when users move up the rankings.
...
### Displaying Leaderboard Rankings {#displaying-leaderboard-rankings}

To fetch a leaderboard and its most up to date rankings, use the [leaderboard API](/en/api-reference/endpoints/leaderboards/get-leaderboard).
...
### Displaying User Rank History {#displaying-user-rank-history}

Use the [user leaderboard API](/en/api-reference/endpoints/users/get-a-users-leaderboard) to fetch data on how a specific user's rank has changed over time in a particular leaderboard.
