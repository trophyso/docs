...
## Trophy Setup {#trophy-setup}

In Trophy, [Metrics](/en/platform/metrics) are the building blocks of gamification and model the different interactions users make with your product.
...
Once you've created your metric, head to the [achievements page](https://app.trophy.so/achievements) and create the achievements you want. You can find all the details on the types of achievements and the different use cases in the [achievements docs](/en/platform/achievements).
...
We've also set up a few achievements related to [Streaks](/en/platform/streaks), but we won't go into detail on these in this guide.

<Tip>
  For a full guide on adding a streaks feature to your web or mobile app, check
  out our [full guide](/en/guides/how-to-build-a-streaks-feature).
</Tip>

In Trophy you track user interactions by sending [Events](/en/platform/events) from your code to Trophy APIs against a specific metric.
...
## Installing Trophy SDK {#installing-trophy-sdk}

To interact with Trophy from your code you'll use the Trophy SDK available in most major [programming languages](/en/api-reference/client-libraries).
...
To track an event (user interaction) against your metric, use the [metric change API](/en/api-reference/endpoints/metrics/send-a-metric-change-event).
...
We can use the response of the [metric change API](/en/api-reference/endpoints/metrics/send-a-metric-change-event) to show pop-up notifications (or 'toasts') when users complete achievements.
...
### Displaying User Achievements {#displaying-user-achievements}

To fetch all achievements a user has completed, use the [user achievements API](/en/api-reference/endpoints/users/get-a-users-completed-achievements).
...
### Displaying All Achievements {#displaying-all-achievements}

If instead you want to display all achievements you've set up in Trophy as part of a globally accessible UI that isn't linked to a particular user, you can use the [all achievements API](/en/api-reference/endpoints/achievements/all-achievements).

<AllAchievementsRequestBlock />

### Achievement Completion Stats {#achievement-completion-stats}
