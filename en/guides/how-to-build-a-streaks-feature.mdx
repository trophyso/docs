...
You can also add multiple metrics and choose whether users must meet all of them or just one—see the [streak conditions docs](/en/platform/streaks#streak-conditions) for details.
...
We can use the response of the [metric change API](/en/api-reference/endpoints/metrics/send-a-metric-change-event) to show pop-up notifications (or 'toasts') when user extends their streak.
...
### Displaying User Streaks {#displaying-user-streaks}

To fetch data on a user's streak use the [user streak API](/en/api-reference/endpoints/users/get-a-users-streak).
...
Learn more about streak freezes in the dedicated [streak freezes docs](/en/platform/streaks#streak-freezes).
