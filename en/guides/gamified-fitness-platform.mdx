...
If you want to skip straight to the code, check out the [example repository](https://github.com/trophyso/example-fitness-platform) or the [live demo](https://fitness.examples.trophy.so).
...
### 2. The Attributes {#2-the-attributes}

To enable local leaderboards, we'll tag every user with a `city` attribute.

## How Trophy Works {#how-trophy-works}

Before diving into the code, let's understand how Trophy powers our gamification layer. In Trophy, [Metrics](/en/platform/metrics) represent different interactions users can make and drive features like [Achievements](/en/platform/achievements), [Streaks](/en/platform/streaks), and [Emails](/en/platform/emails).
...
<AccordionGroup>
  <Accordion title="Create metrics" icon="box">
  ...
  <Accordion title="Create attributes" icon="tag">
    While still on the [metrics page](https://app.trophy.so/metrics), set up
    ...
    Then, go to the [user attributes page](https://app.trophy.so/users/attributes)
...
<Accordion title="Create achievements" icon="trophy">
  Head into the Trophy [achievements page](https://app.trophy.so/achievements)
  and create milestone achievements for each sport. For example:
...
<Accordion title="Configure leaderboards" icon="chart-bar">
  Head to the [leaderboards page](https://app.trophy.so/leaderboards) and set up
...
<Accordion title="Set up XP points" icon="star">
  Head to the [points page](https://app.trophy.so/points) and create a points
  system called `xp` that awards points based on activity: 
  ...
  Then configure **[Points Levels](/en/platform/points#points-levels)** on the same `xp` system so Trophy can track each user's tier. Use the keys and thresholds below so the code in this guide lines up with your dashboard:

  | Name    | Key       | Points threshold |
  | ------- | --------- | ---------------- |
  | Rookie  | `rookie`  | 0                |
  | Active  | `active`  | 100              |
  | Mover   | `mover`   | 500              |
...
<Accordion title="Configure daily streak" icon="flame">
  Head to the [streaks page](https://app.trophy.so/streaks) and configure a
  ...
  <Accordion title="Configure emails" icon="mail">
    Head to the [emails page](https://app.trophy.so/emails) and add and activate templates for these automated engagement emails:
    ...
    Each email type has its own section—add a template under each and activate it to start sending. Configure your branding in the [branding page](https://app.trophy.so/branding) for professional emails.
  </Accordion>
</AccordionGroup>
...
Trophy stores each user's tier on the `xp` points system when you configure [Points Levels](/en/platform/points#points-levels). The [user points](/en/api-reference/endpoints/users/get-a-users-points) payload includes a `level` object. The [list levels API](/en/api-reference/endpoints/points/get-points-levels) returns every tier and threshold so you can show progress toward the next one.

Add a small helper next to your app constants in `src/lib/constants.ts`:
...
The dashboard aggregates all user stats. We fetch data server-side and derive progress bars from thresholds returned by the API.
...
The Log Workout button opens a dialog where users select their activity type, enter distance, and submit. This triggers the `logActivity` server action which sends the event to Trophy.
...
You now have a fully functional fitness gamification loop! Users can log workouts across multiple sports, progress through points levels, earn badges, and compete on leaderboards.
