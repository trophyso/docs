...
Once created, you'll be taken to the configure page for the energy system where you can create [points triggers](/en/platform/points#types-of-triggers) for each of the ways you want to grant or consume energy.
...
Use 'time' triggers to grant users with new energy on an hourly or daily basis, and use [other types](/en/platform/points#types-of-triggers) of triggers with negative values to consume energy from the different user interactions you want.
...
## Metering Usage {#metering-usage}

To prevent users from taking actions in your product based on energy, use the [user points API](/en/api-reference/endpoints/users/get-a-users-points) to fetch their current total energy.
...
## Displaying Energy {#displaying-energy}

To fetch a users energy use the [user points API](/en/api-reference/endpoints/users/get-a-users-points).

<UserPointsRequest />

This API returns data on the user's total energy but can be configured to also return between 1 and 100 of the user's most recent energy changes by using the `awards` [query parameter](/en/api-reference/endpoints/users/get-a-users-points#parameter-awards).

<UserEnergyResponse />

The [user points summary API](/en/api-reference/endpoints/users/get-a-users-points-summary) can also be used to drive chart-based UI, like showing users their energy usage over time.
