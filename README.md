...
- **OpenAPI specs (local files)**: Keep checked-in specs at `openapi/application.json` (Application API + webhooks) and `openapi/admin.json` (Admin API). Register them under top-level `api.openapi` in `docs.json`. Pin the file path in each page’s frontmatter (do **not** set `openapi` on Application API / Admin API / Webhooks tabs—Mintlify auto-populates and duplicates MDX endpoints; do **not** put remote `https://…` URLs in frontmatter—Mintlify doesn’t reliably resolve those). Forms: `openapi: "/openapi/application.json GET /users/{id}"`, `openapi: "/openapi/application.json webhook points.changed"`, `openapi: "/openapi/admin.json METHOD /path"`. Keep `openapi:` lines unchanged across locales; Lingo must not alter them. Refresh from live URLs with **`npm run openapi:pull`** (`scripts/pull-openapi-specs.mjs`). After pulling, run **`npm run translate:sync-openapi-titles`** if operation summaries may have changed.
...
| `openapi:pull` | Download live OpenAPI specs into `openapi/application.json` and `openapi/admin.json`. |
