---
title: Logros
description: Usa los Logros de Trophy para premiar a los usuarios al completar hitos, desbloquear insignias y alcanzar objetivos dentro de tu producto gamificado.
...
Los logros de métricas están vinculados a [Métricas](/es/features/metrics) y se utilizan mejor cuando quieres incentivar a los usuarios a realizar la misma acción una y otra vez.
...
Cuando se desbloquean logros, Trophy incluye información sobre los logros desbloqueados en la respuesta del [API de Eventos](/es/api-reference/endpoints/metrics/send-a-metric-change-event) y activa automáticamente los [Correos de Logros](/es/features/emails#achievement-emails) si están configurados.
...
Al igual que los logros de métricas, los logros de API también pueden activar [Correos de Logros](/es/features/emails#achievement-emails) automatizados si están configurados.

### Logros de Racha {#streak-achievements}

Los logros de racha están directamente vinculados a la [Racha](/es/features/streaks) de un usuario y se desbloquean automáticamente cuando los usuarios alcanzan una longitud de racha determinada.
...
Si utilizas logros de métricas, no es necesario _completar_ logros explícitamente. Una vez que hayas configurado el [seguimiento de métricas](/es/features/events#tracking-metric-events) en tu código, todos los logros vinculados a la métrica se rastrearán automáticamente.
...
Si tienes logros configurados para cualquiera de tus [Métricas](/es/features/metrics), la página de analítica de métricas muestra un gráfico con el progreso actual de todos los usuarios de la siguiente manera:
...
<AccordionGroup>
  <Accordion title="¿Qué tipo de logros debo usar?">
    ...
    Usa la [analítica de métricas](/es/features/metrics#metric-analytics) de Trophy para comparar la retención de cada interacción de usuario y luego configura logros en torno a esas interacciones para maximizar el impacto en la retención.
