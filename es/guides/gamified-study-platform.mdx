...
En Trophy, las [Métricas](/es/features/metrics) representan diferentes interacciones que los usuarios pueden realizar y pueden impulsar funcionalidades como [Logros](/es/features/achievements), [Rachas](/es/features/streaks) y [Correos electrónicos](/es/features/emails).
...
En Trophy rastreamos las interacciones de los usuarios enviando [Eventos](/es/features/events) desde nuestro código a Trophy contra una métrica específica.
...
Registrar un evento contra una métrica para un usuario específico utiliza la [API de eventos de métricas](/es/api-reference/endpoints/events/submit-a-metric-event) de Trophy, que en la práctica se ve así:
...
<Tip>
  Descarga y utiliza este archivo zip de insignias (listo para esta aplicación
  de ejemplo):
...
<Note>
  Antes de poder comenzar a enviar correos, necesitarás [configurar tu dirección de envío](/es/features/emails#sending-emails) con Trophy. Pero esto puede hacerse más adelante si lo prefieres.
</Note>
