...
En Trophy realizas seguimiento de las interacciones de usuario enviando [Eventos](/es/platform/events) desde tu código a las API de Trophy contra una métrica específica.
...
## Seguimiento de Interacciones de Usuario {#tracking-user-interactions}

Para realizar seguimiento de un evento (interacción de usuario) contra tu métrica, utiliza la [API de cambio de métrica](/es/api-reference/endpoints/metrics/send-a-metric-change-event).
...
Podemos usar la respuesta de la [API de cambio de métrica](/es/api-reference/endpoints/metrics/send-a-metric-change-event) para mostrar notificaciones emergentes (o 'toasts') cuando los usuarios completen logros.
...
### Mostrar Logros del Usuario {#displaying-user-achievements}

Para obtener todos los logros que un usuario ha completado, usa la [API de logros de usuario](/es/api-reference/endpoints/users/get-a-users-completed-achievements).
...
Si en cambio deseas mostrar todos los logros que has configurado en Trophy como parte de una interfaz globalmente accesible que no esté vinculada a un usuario en particular, puedes usar la [API de todos los logros](/es/api-reference/endpoints/achievements/all-achievements).
