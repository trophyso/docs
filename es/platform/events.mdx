...
Los eventos representan interacciones individuales de usuarios con [Métricas](/platform/metrics) en Trophy. Un evento corresponde a una única interacción realizada por un único usuario.
...
#### Activadores de Características Avanzadas
...
Si utilizas cualquier [correo electrónico](/platform/emails) de Trophy, los atributos de eventos se pueden usar para personalizar los datos mostrados en ciertos bloques de correo.
...
Para empezar a hacer seguimiento de las interacciones de los usuarios como eventos en tus Métricas de Trophy, utiliza la [API de Métricas](/api-reference/endpoints/metrics/send-a-metric-change-event) o una de nuestras [bibliotecas cliente](/api-reference/client-libraries) con tipado seguro, compatibles con la mayoría de los lenguajes de programación principales.
...
Mira a Charlie integrar el seguimiento de métricas en una aplicación simple de NextJS usando el SDK de [Node.js](/api-reference/client-libraries) de Trophy:
...
Para usar eventos idempotentes, utiliza el encabezado `Idempotency-Key` en la [API de eventos de métricas](/api-reference/endpoints/metrics/send-a-metric-change-event).

[Más información sobre idempotencia](/api-reference/idempotency).
