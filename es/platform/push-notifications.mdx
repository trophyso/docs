...
og:description: Usa flujos automatizados de notificaciones push para extender tu
  experiencia de gamificación fuera de tu app y recordar a los usuarios que
  sigan regresando.
icon: bell
---
...
- Las **notificaciones de logros** se envían a los usuarios cada vez que desbloquean un [Logro](/es/platform/achievements).
...
- Las **notificaciones de rachas** se envían automáticamente a los usuarios recordándoles extender su [Racha](/es/platform/streaks).
...
Expo Push Service te evita tener que integrar FCM y APNs individualmente y dirige automáticamente las notificaciones a través del servicio relevante según la plataforma de tus usuarios.
...
> </video>
  ...
  <Step title="Activar plantillas">
    ...
    <Tip>
      Los usuarios pueden controlar qué tipos de notificaciones push reciben a través de la [API de preferencias de usuario](/es/platform/users#notification-preferences) de Trophy. Cuando un usuario ha desactivado un tipo de notificación en sus preferencias, Trophy respetará esa configuración y no enviará esas notificaciones a ese usuario.
    </Tip>
...
### Estructura de Plantilla {#template-structure}
...
### Usar Variables {#using-variables}
