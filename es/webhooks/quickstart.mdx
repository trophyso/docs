...
<Note>
  ...
  de Trophy](/getting-started/quickstart) para comenzar.
</Note>
...
Trophy enviará solicitudes a tu manejador, indicándole qué `type` de evento está enviando. Esto te permite construir un manejador que puede procesar múltiples tipos de eventos.
    ...
    <Warning>
    Asegúrate de que tu manejador de webhook esté configurado para devolver un código de estado `200`. Si Trophy detecta que tu manejador devuelve un código de estado que no es `2XX`, lo tratará como un fallo e [intentará la solicitud nuevamente](/webhooks/retries).
    </Warning>
