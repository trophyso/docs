...
Trophy's [event tracking API](/api-reference/endpoints/events/submit-a-metric-event) can be used to enforce idempotency preventing client retries from having unintended side effects such as overcounting user interactions, or awarding points to users multiple times for the same action.
...
To ensure idempotency is respected when sending events to Trophy, include an `Idempotency-Key` header when using the [event tracking API](/api-reference/endpoints/events/submit-a-metric-event). Additionally, all [client SDKs](/api-reference/client-libraries) support idempotency with built-in type safety.
