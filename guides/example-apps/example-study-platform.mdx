...
The example project also uses daily [streaks](/platform/streaks) where users must look at least one flashcard a day to keep their streak. For each consecutive day the user looks at least one flashcard, their streak increases. But if they miss a day, they lose their streak and have to start over.
...
The example app has an XP systems built on Trophy's [points](/platform/points) feature that rewards users with different numbers of XP for taking different actions:

- 50 XP for every 7 day streak
- 5 points for every flashcard viewed
- 10 bonus points for every 10 flashcards viewed

### Automated Email Sequences

The example app also has built in [email sequences](/platform/emails) to automatically send notifications to users to based on the following use cases:
...
To make all this possible, the [tutorial](/guides/tutorials/how-to-build-a-gamified-study-platform) walks through creating the necessary resources in a new Trophy account.

If instead you just want to get started building your own gamification experience, follow the [quick start guide](/getting-started/quickstart).
