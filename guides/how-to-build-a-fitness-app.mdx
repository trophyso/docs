...
- Add **achievement unlock toasts** using the `achievements` array from the [metric event response](/api-reference/endpoints/metrics/send-a-metric-change-event)
- Add **sound effects** for achievements and streak extensions
- Configure [automated emails](/platform/emails) for streak reminders and weekly recaps
