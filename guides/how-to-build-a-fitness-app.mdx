...
<AccordionGroup>
  ...
  <Accordion title="Create achievements" icon="trophy">
    Create milestone achievements for each sport. For example:
  ...
  <Accordion title="Configure leaderboards" icon="chart-bar">
    Set up weekly leaderboards to drive competition:
  ...
  <Accordion title="Set up XP points" icon="star">
    Create a points system called `xp` that awards points based on activity:
  ...
  <Accordion title="Configure emails" icon="mail">
    Enable automated engagement emails:
...
The real magic happens when users receive instant feedback. Let's add toast notifications and sound effects when achievements unlock or streaks extend.

### Achievement Toasts

First, add the `sonner` component from shadcn/ui:

```bash
npx shadcn@latest add sonner
```

Create a custom toast component for achievements:

```tsx lib/toast.tsx [expandable]
"use client";
...
function Toast({ title, description, image }: ToastProps) {
  return (
    <div className="flex rounded-lg bg-white shadow-lg ring-1 ring-black/5 w-full md:max-w-[364px] items-center p-4">
      {image && (
        <Image src={image.src} alt={image.alt} width={40} height={40} className="mr-4 rounded-md" />
      )}
      <div className="flex-1">
...
Sound effects dramatically increase engagement. Add audio feedback for achievements and streaks:

```tsx components/log-activity-dialog.tsx [expandable]
// Inside your activity logging component
      ...
      if (response.data.achievements?.length) {
        ...
        response.data.achievements.forEach((achievement) => {
          toast({
            ...
            image: { src: achievement.badgeUrl, alt: achievement.name },
          });
        });
      }
      
      // Handle streak extensions
      if (response.data.currentStreak?.extended) {
        streakSound.current?.play();
        toast({
          title: "🔥 Streak Extended!",
          description: `You're on a ${response.data.currentStreak.length} day streak!`,
        });
      }
    }
  };
...
<Tip>
  Get free sound effects from [freesound.org](https://freesound.org). Short, satisfying chimes work best for achievements.
</Tip>

### Configuring Trophy Image Host

To display achievement badges, configure Trophy's image host in `next.config.ts`:
...
You now have a fully functional fitness gamification loop! Users can log workouts across multiple sports, level up their profile, earn badges, compete in local leagues, and receive satisfying audiovisual feedback for their progress.
...
- **Daily streaks** with engaging toast notifications
- **Engaging UX** with sound effects and visual feedback

### Next Steps

- Add [automated emails](/platform/emails) for streak reminders and weekly recaps
- Implement [challenges](/platform/challenges) for time-bound competitions
- Add social features like friend comparisons
