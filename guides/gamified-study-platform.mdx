...
In this tutorial we'll build an example study platform using Trophy for gamification. If you want to just skip to the end then feel free to check out the [template repository](https://github.com/trophyso/example-study-platform) or the [live demo](https://study.examples.trophy.so).
...
- [Adding Gamification](#adding-gamification)
...
In Trophy, [Metrics](/platform/metrics) represent different interactions users can make and can drive features like [Achievements](/platform/achievements), [Streaks](/platform/streaks) and [Emails](/platform/emails).
...
<Note>
  ...
  address](/platform/emails#setting-up-emails) with Trophy. But this can be done
...
<Frame>
  <img
    ...
    width="100%"
    noZoom
    src="../assets/guides/example-apps/example-study-app/points-setup.png"
  />
</Frame>
...
export function UserPointsProvider({ children }: { children: ReactNode }) {
  ...
  const [lastPoints, setLastPoints] = useState<GetUserPointsResponse | null>(null);
  ...
  return (
    <UserPointsContext.Provider value={{ points, lastPoints, loading, error, refetch }}>
      {children}
...
Finally, refetch points whenever a user views a flashcard to show real-time XP updates:
...
<Frame>
  <video
    ...
    playsInline
    className="w-full aspect-video"
    src="../assets/guides/example-apps/example-study-app/points-demo.mp4"
  ></video>
</Frame>
...
For even more features like points history charts, leaderboard journey views, and additional UI polish, check out the [live demo](https://study.examples.trophy.so) and the [GitHub repo](https://github.com/trophyso/example-study-platform).

You could also extend this further with:
- **Challenges** - Create time-bound study challenges with Trophy's upcoming challenges feature
- **Social features** - Let users compete with friends or study groups
- **Rewards** - Integrate with gift card APIs to offer real prizes for top performers
