...
Next up, we'll fetch the user's streak and achievements and display them in a dedicated dialog. For this we'll first need to add the shadcn/ui `<Dialog />` compnent to our project:

```bash
npx shadcn@latest add dialog
```

Next, we'll add a couple of new server actions to fetch the users streak and achievements from Trophy. As we're using a daily sterak here, we'll fetch the last 14 days of streak data from Trophy to give us enough to work with in our UI:
...
Next, we'll call these actions at request-time.
