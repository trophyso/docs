  ...
  <Tab title="Using AI" icon="bot">
    ...
    <Steps>
      ...
      <Step title="Configure MCP servers">
        ...
        - Account MCP server URL: `https://mcp.trophy.so` (account actions — see [MCP](/mcp/overview); requires `x-trophy-api-key`)
      ...
      <Step title="Tell your agent to perform initial setup">
        ...
        >
          ...
          3. Add code to track a metric change event for the metric `<metric>` using the Trophy API client, wiring all required user attributes like ID, and using placeholders for all optional attributes like name, email, tz and signUpDate.
          ...
          Constraints:
          - Use Docs MCP tool results for all Trophy-specific implementation decisions.
          - Do not rely on prior memory for Trophy API behavior when MCP provides an answer.
      ...
      <Step title="Build your first gamification feature">
        ...
        >
          ...
          First, verify the connected Trophy Docs MCP server is available and use it as the source of truth for feature-specific integration details.

          Before implementing anything, ask me which gamification feature I want to build, providing options based on what the Docs MCP says Trophy supports.

          After I choose one option:
          1. Configure the selected feature in the way I describe using the Account MCP server.
