...
Leaderboards in Trophy can be configured to group users into smaller subsets according to a specific [custom user attribute](#custom-user-attributes).

<Tip>
Where possible we consider this best practice as smaller, more socially-connected leaderboards often have higher engagement than generic leaderboards. This is why we [limit](#participant-limits) leaderboards to a maximum of 1,000 participants.

If you think you want to create a leaderboard with a higher number of participants than this, instead consider using breakdowns.

</Tip>

On the leaderboard configuration page, create or select your user attribute in the 'Breakdown Attribute' field. Trophy will automatically start grouping users into smaller leaderboards based on the values of your chosen attribute for each of your users.
...
<Tip>
We chose to limit leaderboard size to help guide customers on best practice. Traditionally, leaderboards with lots of participants fail to truly engage users beyond the top 1%. Therefore imposing a maximum helps create an engaging user experience.

If you think you need a leaderboard longer than 1,000 participants, consider using [leaderboard breakdowns](#ranking-breakdowns) instead.

For more background on our thinking around this, read this [blog post](https://www.trophy.so/blog/leaderboard-only-motivating-top-one-percent).
