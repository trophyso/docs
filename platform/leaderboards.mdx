...
Leaderboarsd in Trophy have a number of statuses to help you control when users and how users can join them.
...
All new leaderboards are created as `Inactive`. Whilst inactive, any properties or settings of the leaderboard can be changed, they won't be visible to users and users can't join them.
...
If a leaderboard has an [end date](#end-dates), then once it's past Trophy will automatically move it to `Finished` status and stop monitoring user activity. Once a leaderboard has finished it won't be visible to users but you can still query APIs to get rankings for historical runs.

If you decide you no longer need a leaderboard, you can move it to `Archived` status. Only leaderboards in `Inactive` or `Finished` status can be archived.
