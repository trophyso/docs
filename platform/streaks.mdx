...
You can configure any number of arbitrary freezes to grant to new users when you identofy them with Trophy.
...
Monitor retention on the [streaks dashboard](https://app.trophy.so/streaks) in Trophy and experiment with granting different numbers of freezes.
...
If you've [configured time zones](/platform/users#param-tz) against users, Trophy will grant freezes to users on a schedule in keeping with their local clock.

### Maximum Freeze Count

You can also configure the maxiumum number of freezes that each user can have. If set, [freeze accumulation](#freeze-accumulation) will only ever grant freezes up to this limit.
