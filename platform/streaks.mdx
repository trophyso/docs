...
You can configure the number of freezes that you want users to start with, and that new users you tell Trophy about will get automatically.

### Freeze Accumulation

Trophy can automatically accumulate streak freezes on users behalf based on your preference. You can choose an arbitrary number of days over which to grant an arbitrary number of freezes to each user.

If you've [configured time zones](/platform/users#param-tz) for your users, Trophy will gran freezes to users according to their local clock.

### Maximum Freeze Count

You can also configure the maxiumum number of freezes that each user can have. If set, [freeze accumulation](#freeze-accumulation) will only grant freezes up to this limit for each user.
