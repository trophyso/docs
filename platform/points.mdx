...
Points boosts are multipliers that you can use to incentivise user actions in your product during a specific time period.

This section walks through how boosts work, and how you can use them to incentivise users to take actions within your application and increase retention and engagement.

### Boost Targeting

There are a few different ways you can use boosts in Trophy.

- **Global boosts** multiply points earned by all users during the boost window.
- **Scoped boosts** using [custom user attributes](/platform/users#custom-user-attributes) only multiply points earned by a particular user group with custom attribute values matching those set on the boost.
- **User-specific boosts** only multiply points earned by a single user within the boost window.

Typically, global boosts are used to increase platform-wide engagement during key calendar events like BFCM in e-commerce, New Year in fitness and exam season in ed-tech platforms.

Conversely, scoped and user-specific boosts are commonly targeted at providing additional incentives to specific usage patterns that are leading to churn or disengagement.

### Creating Boosts

<Note>
User-specific boosts can only be created programatically through the [Admin API](/admin-api/endpoints/points/create-boosts).
</Note>

To create a points boost via the Trophy dashboard, follow the steps below.
...
<Steps>
  <Step title="Head to boosts page">
    Head to the [points page](https://app.trophy.so/points) and click on the points system you want to create the boost for. Then head to the boosts tab and hit the *New Boost* button.
  </Step>
  ...
  <Step title="Set Multipler">
    Set the numeric value that the boost will multiply points earned by. If you choose a decimal multipler, you'll have the option to choose the [rounding behavoir](#boost-rounding) the boost should use.
  </Step>
  <Step title="Set Boost Window">
  ...
  <Step title="Activate Boost">
    Hit *Save Boost* and make your boost active when ready.
  </Step>
</Steps>
...
Boost multiplers must be positive numbers, but can be decimals to support secnarios where percentage boosts like '50% more points' may be required.

### Boost Stacking

Points boosts in Trophy stack through [multiplication](#boost-multiplers). This is to support allowing users to benefit from multiple boosts simultaneously.

To demonstrate stacking consider a 2X, 1.5X and a 3X boost all active within the same time period. The resulting boost multipler for a user that qualifies for all three boosts would be as follows:

```txt Boost Stacking Example
Overall Multipler (3 boosts) = 2X * 1.5X * 3X = 9X
```
...
However, when using decimal [boost multiplers](#boost-multiplers), there may be some scenarios where this default rounding is not enough to always produce integer points values.

Therefore Trophy offers three rounding modes to provide additional control on rounding behavoir specifically when boosts are involved in points calculations.
...
- **Round Up**: Trophy rounds decimal points up to the nearest integer after boost multiplcation.
- **Round Nearest**: Trophy will round points to the nearest integer, up or down, based on the final value after boost multiplcation. For example `1.2` is rounded to `1` and `1.7` is rounded to `2`.

<Tip>
In scenarios where points are used to mimic platform credit mechanics, it's recommended to use the default rounding down behavoir to protect against slippage.
</Tip>
