...
The response is an array of levels with `key`, `name`, `description`, optional `badgeUrl`, and the `points` threshold for each tier. Sort by `points` ascending in your client if you want a clear ladder from lowest to highest threshold.

The data represents stable configuration based on your Trophy points system so binding UI to this will ensure it automatically updates as your levels evolve in Trophy:

<PointsLevelsListResponse />

#### Displaying User Level

The [user points API](/api-reference/endpoints/users/get-a-users-points) returns the user’s `total`, their current `level` object, and recent `awards`. The `level` field is the tier Trophy has computed for them right now, or `null` if levels are not configured on that system or they have not reached a tier yet.
