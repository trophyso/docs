...
First, the [user points API](api-reference/endpoints/users/get-a-users-points) returns the user's total points and up to 100 of the most recent events that awarded points to the user.
...
First, the [points summary API](/api-reference/endpoints/points/get-points-summary) returns aggregate points data across your entire user base.
...
Finally, the [points triggers API](/api-reference/endpoints/points/get-all-points-triggers) returns data about the active triggers that award points to users in your Trophy account.
