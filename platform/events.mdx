...
Any [Achievements](/platform/achievements), [Streaks](/platform/streaks), [Points](/platform/points) or [Leaderboards](/platform/leaderboards) that have been set up against this metric will be automatically processed, and the response will contain any updates to the users progress that are a direct result of the event occurring:

<MetricChangeResponseBlock />

In this example the response includes the following:

- The users newly unlocked achievements as a result of the event
- The users latest streak as result of the event
- The users latest points for each points system that changed as a result of the event
- The users latest leaderboard data for each leaderboard that changed as a result of the event
