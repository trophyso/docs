...
## What are Leaderboards? {#what-are-leaderboards}
...
## Types of Leaderboards {#types-of-leaderboards}
...
### Repeating Leaderboards {#repeating-leaderboards}
...
#### Tips for Weekly Leaderboards {#tips-for-weekly-leaderboards}
...
#### Metric Rankings {#metric-rankings}
...
#### Streak Rankings {#streak-rankings}
...
### Ranking Breakdowns {#ranking-breakdowns}
...
In addition to [start & end dates](#start-end-dates), leaderboards can be configured to have time restrictions.

When time restrictions are enabled, leaderboard rankings will only update during the configured time period.

Time restrictions are useful when building leaderboards for limited-time events or promotions that run for a specific period of time.

Time restrictions work on all leaderboards, including [repeating leaderboards](#repeating-leaderboards). Where a repeating leaderboard has a time restriction, the time restriction will be applied to each run of the leaderboard.

### Setting Time Restrictions {#setting-time-restrictions}

To set time restrictions on a leaderboard, head to the leaderboard configuration page and turn on the switch labelled 'Restrict to time window'. This will give you two fields, a start and end time.

- Setting only a start time will mean leaderboard rankings only start to update after the start time on each day that the leaderboard is active.
- Similarly, setting only an end time will mean leaderboard rankings will stop updating after the end time on each day that the leaderboard is active.
- Setting both a start and end time will mean leaderboard rankings will only update during the configured time period.
...
Time restrictions on leaderboards are applied in **local time** for each participant based on their configured [time zone](/features/users#param-tz).

This means that each user can participate in the leaderboard at any time during the configured time restriction according to their local clock.

## Participant Limits {#participant-limits}

Leaderboards in Trophy have a default maximum number of participants of **1,000**. However a leaderboard can be configured to have any arbitrary number of participants to support use cases like _Top 100_ or similar.
...
<Tip>
If you require a larger participant limit, considering using [breakdown attributes](#ranking-breakdowns) to group participants into smaller cohorts. Or please contact us at [support@trophy.so](mailto:support@trophy.so) and we'll be happy to increase your defaults.
</Tip>
...
## Displaying Leaderboards {#displaying-leaderboards}
...
### Total Unique Participants {#total-unique-participants}
