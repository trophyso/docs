...
Events represent individual user interactions against [Metrics](/features/metrics) in Trophy. One event corresponds to a single interaction made by a single user.
...
#### Advanced Feature Triggers {#advanced-feature-triggers}
...
<CardGroup>
  <Card
    title="Achievement Triggers"
    icon="trophy"
    href="/features/achievements#creating-achievements"
  >
    Configure achievements that can only be unlocked by events with certain
    attribute values.
  </Card>
  <Card
    title="Points Triggers"
    icon="sparkle"
    href="/features/points#points-triggers"
  >
...
If you use any Trophy [Emails](/features/emails), event attributes can be used to customize the data shown in certain email blocks.

Firstly, when using metric-based variables in email copy you can use event attributes to further control what data the variable references.
...
To start tracking user interactions as events against your Trophy metrics, use the [Metrics API](/api-reference/endpoints/events/submit-a-metric-event) or one of our type-safe [Client SDKs](/api-reference/client-libraries) supported in most major programming languages.

If you're sending events in bulk from your backend (for example queue flushes, migrations, or batch imports), use the Admin API endpoint [Submit a batch of metric events](/admin-api/endpoints/events/submit-a-batch-of-metric-events) to send up to 1,000 metric events in one request.
...
Any [Achievements](/features/achievements), [Streaks](/features/streaks), [Points](/features/points) or [Leaderboards](/features/leaderboards) that have been set up against this metric will be automatically processed, and the response will contain any updates to the user's progress that are a direct result of the event occurring:
...
To use idempotent events, use the `Idempotency-Key` header in the [metric event API](/api-reference/endpoints/events/submit-a-metric-event).

[Learn more about idempotency](/api-reference/idempotency).

### Parallel Events {#parallel-events}

Trophy does not support parallel events to same metric for the same user. Instead, use the `value` parameter to increment the metric by the total amount of all individual events that you need to track.
