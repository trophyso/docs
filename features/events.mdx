...
If you're sending events in bulk from your backend (for example queue flushes, migrations, or batch imports), use the Admin API endpoint [Submit a batch of metric events](/admin-api/endpoints/metrics/submit-a-batch-of-metric-events) to send up to 1,000 metric events in one request.
