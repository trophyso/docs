...
<ParamField path="signUpDate" type="string">
  The date the user signed up on your platform, as `YYYY-MM-DD`. ISO 8601
  date-times are accepted and stored as their UTC calendar day. Must not be
  after today in the user's timezone. This field is separate from
  `created`, which is the date Trophy first learned about the user.
</ParamField>
...
Attributes can be assigned values for specific users using their unique key either inline, as users increment metrics through the [metric increment API](/api-reference/endpoints/events/submit-a-metric-event), or explicitly through the [user identification](/api-reference/endpoints/users/identify-a-user), [create user](/api-reference/endpoints/users/create-a-user), or [update user](/api-reference/endpoints/users/update-a-user) APIs.
...
<CardGroup>
  ...
  <Card
    ...
    icon="split"
    href="/features/emails#limiting-emails-to-specific-types-of-users"
  >
  ...
  <Card
    ...
    icon="trophy"
    href="/features/emails#email-variables"
  >
...
In practice this means whenever you use the [Metric Event API](/api-reference/endpoints/events/submit-a-metric-event), you pass along full details of the user who triggered the event.
...
- `enabled`: Whether streaks are calculated for this user. When `false`, the user's streak is always `0` and streak webhooks and notifications are not sent.
