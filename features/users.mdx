...
Trophy supports user preferences for streaks, allowing users to customize the conditions they must meet to extend their streak, as well as personalize what days of the week that their streak will track their activity.
...
- `daysOff`: An array of ISO 8601 days numbers that the user will not lose their streak on if inactive.
